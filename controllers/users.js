...
let userController = {
  ...
  login: async (req, res) => {
    try {
      ...
      const verifyLink = `http://www.nexonapp.com/users/verify-login?token=${token}&userId=${user._id}`;
...
sendRecoveryEmail: async (req, res) => {
  try {
      ...
      const verifyLink = `http://www.nexonapp.com/users/updated-email?token=${user.emailVerificationToken}`;
...
<body>
    <div class="container">
        ...
        <p><a href="http://www.nexonapp.com/update-email" class="boton"><strong>Cambiar mi correo</strong></a></p>
